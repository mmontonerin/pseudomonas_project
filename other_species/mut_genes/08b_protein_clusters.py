...
def find_unique_protein_clusters(mut_genes, input_directory, output_directory, csv_file, output_csv, prefix):
    # Ensure output directory exists
    os.makedirs(f"{output_directory}/{prefix}", exist_ok=True)

    # Read the CSV file
    csv_file = f"{prefix}_master_table_precluster.csv"
    output_csv = f"{prefix}_master_table_clusters.csv"
    print(f"Processing CSV file: {csv_file}")
    df = pd.read_csv(csv_file, dtype={'num': str})  # Ensure 'num' is read as a string
    
    for gene in mut_genes:
        fasta_file = f"{input_directory}/{prefix}/{gene}_id.fasta"
        output_fasta = f"{output_directory}/{prefix}/{gene}_clusters.fasta"
        ...
        print(f"Processing FASTA file for {gene} ({prefix})...")
        ...
        df[column_name] = df[f'{prefix}_num'].apply(find_cluster)
        
        # Debugging: Check for unmatched IDs
        unmatched = [num for num in df[f'{prefix}_num'] if num not in sequence_to_cluster]
        print(f"\nUnmatched IDs in {gene} CSV for {prefix}:", unmatched)
...
input_directory = "/home/montoliu/other_species/mut_genes/protein_analysis"
output_directory = "/home/montoliu/other_species/mut_genes/protein_analysis/clusters"

# Process each prefix
for prefix in prefixes:
    csv_file = f"{prefix}_master_table_precluster.csv"
    output_csv = f"{prefix}_master_table_clusters.csv"
    find_unique_protein_clusters(mut_genes, input_directory, output_directory, csv_file, output_csv, prefix)
