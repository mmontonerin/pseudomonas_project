...
# Read NCBI table in
df_ncbi = pd.read_csv('pvir.csv', encoding='ascii', sep=',')

# Read filenames table, final table to be built from here
df_fn = pd.read_csv('pvir_assembly_names.txt', encoding='ascii', header=0)
...
def extract_strain(row):
    ...
    if accession in accession_to_values:
        ...
        for col in [column_strain_ncbi, column_isolate_ncbi, column_ecotype_ncbi]:
            ...
            #print(f"Debug: Accession: {accession}, Col: {col}, Strain: {strain}")  # Debugging output
            if pd.notna(strain):
                # Remove spaces and replace with underscores
                strain = str(strain).replace(' ', '_')
...
#quast directory
quast_directory = '/home/montoliu/other_species/data/quast/pvir/stats_all'
...
busco_directory = '/home/montoliu/other_species/data/busco/pvir'
...
df_fn.to_csv('pvir_mastertable_stats.csv', index=False)
