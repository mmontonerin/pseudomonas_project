...
def filter_table(input_file):
    ...
    for index, row in df_fn.iterrows():
        if re.match(pattern_frag, str(row['dnaQ'])): 
            ...
            df_condition_fail.loc[index] = row
        elif re.match(pattern_end, str(row['dnaQ'])): 
            df_condition_fail.loc[index] = row
        elif re.match(pattern_end, str(row['mutT'])):
            df_condition_fail.loc[index] = row 
        elif re.match(pattern_end, str(row['mutL'])):
            df_condition_fail.loc[index] = row
        elif re.match(pattern_end, str(row['mutS'])):
            df_condition_fail.loc[index] = row
        elif re.match(pattern_end, str(row['mutY'])):
            df_condition_fail.loc[index] = row
        elif re.match(pattern_end, str(row['uvrD'])):
            df_condition_fail.loc[index] = row
    ...
    df_condition_fail.to_csv('filtered_out_final.csv', index=False)

    # Save unfiltered data to another file
    df_condition_pass.to_csv('final_table_of_genome_assemblies_final.csv', index=False)
...
#    contigs = 400
#    No Ns in genes
#    No genes in the end of contigs (<200 bp till end)
