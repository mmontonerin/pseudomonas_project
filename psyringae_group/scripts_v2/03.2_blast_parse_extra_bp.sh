...
grep -ve 'range -[0-9]+' -ve '--[0-9]+ -strand' ${run}/extract_blast_seq_extrabp.sh > ${run}/extract_blast_seq_extrabp_complete.sh
grep -ve 'range -[0-9]+' -ve '--[0-9]+ -strand' ${run}/extract_blast_seq_frag_extrabp.sh > ${run}/extract_blast_seq_frag_extrabp_complete.sh


# If the range gets negative, turn to 1 instead

sed -i -E 's/range -[0-9]+/range 1/g;s/--[0-9]+ -strand/-1 -strand/g;s/completeness/completeness_endcontig/g' ${run}/extract_blast_seq_extrabp_endcontig.sh
sed -i -E 's/range -[0-9]+/range 1/g;s/--[0-9]+ -strand/-1 -strand/g;s/completeness/completeness_endcontig/g' ${run}/extract_blast_seq_frag_extrabp_endcontig.sh


# Make the script executable
chmod +x ${run}/extract_blast_seq_extrabp_endcontig.sh
chmod +x ${run}/extract_blast_seq_frag_extrabp_endcontig.sh
chmod +x ${run}/extract_blast_seq_extrabp_complete.sh
chmod +x ${run}/extract_blast_seq_frag_extrabp_complete.sh
