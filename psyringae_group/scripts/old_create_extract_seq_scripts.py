...
def extract_sequences(assembly, a_name, g_name, seq, result, run, frag_script=False):
            ...
            for line in infile:
                ...
                if contig_id is None:
                    ...
                    if fields[1] != contig_id:
                        ...
                        print_blastdbcmd(assembly, a_name, g_name, frag_seq, prev_contig_id, min_start, max_end, frag_script)
        ...
        with open(input_file, 'r') as infile:
            for line in infile:
                ...
                if int(fields[10]) > int(fields[9]):
                    print_blastdbcmd(assembly, a_name, g_name, seq, fields[2], fields[9], fields[10], plus_strand=True, frag_script=frag_script)
                else:
                    print_blastdbcmd(assembly, a_name, g_name, seq, fields[2], fields[10], fields[9], plus_strand=False, frag_script=frag_script)

def print_blastdbcmd(assembly, a_name, g_name, seq, contig_id, min_start, max_end, plus_strand=None, frag_script=False):
    if plus_strand is None:
    ...
    if frag_script:
        outfile = f"{run}/extract_blast_seq_frag.sh"
    else:
        outfile = f"{run}/extract_blast_seq.sh"
...
def main():
    ...
    parser.add_argument("--frag", action="store_true", help="Indicate fragmented sequences")

    args = parser.parse_args()

    extract_sequences(args.assembly, args.aname, args.gname, args.seq, args.result, args.run, frag_script=args.frag)
...
def extract_sequences(assembly, a_name, g_name, seq, result, run, frag_script=False):
    ...
    with open(input_file, 'r') as infile:
        has_multiple_lines = len(infile.readlines()) >= 2

    frag_script = has_multiple_lines
